# scripts/phase2/build_dictionary_todo.R
...
# scripts/phase2/merge_dictionary.R to append the completed rows.
#
# Run:  Rscript scripts/phase2/build_dictionary_todo.R
# Reads:  data/case_docket/**            (all parsed dockets)
#         scripts/phase2/lfo_codes.csv   (current dictionary)
# Writes: scripts/phase2/lfo_codes_TODO.csv

options(oscn.autorun.suppress = TRUE)
suppressPackageStartupMessages({ library(arrow); library(dplyr); library(readr) })
source("scripts/phase2/classify_lfo.R")   # classify_docket(), load_dict(), unmapped_codes(), DOCKET_ROOT

TODO_PATH <- "scripts/phase2/lfo_codes_TODO.csv"
